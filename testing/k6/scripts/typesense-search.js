...
export default function () {
  ...
  const url =
    `${TYPESENSE.baseUrl}/collections/${INDEX_NAME}/documents/search` +
    `?q=${encodeURIComponent(term)}&query_by=title,text&per_page=10`;

  const res = http.get(url, { headers: TYPESENSE.headers });
